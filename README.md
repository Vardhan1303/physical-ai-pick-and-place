...
- **Single camera means partial 2.5D geometry.** The point cloud only contains what one camera sees in one frame — mostly the object's top face for the current box placeholder. Grasp height is an estimate refined by close-until-contact / stop-on-stall behavior during execution, not a measurement of the object's hidden geometry.
- **FLIP under-segmentation on the current synthetic placeholder.** In verification runs, FLIP's mask tends to cover mostly the marker's immediate neighborhood rather than the object's full extent (ground-truth IoU around 0.2 on the placeholder box). This looks like a sigma/ROI-scale calibration question, not a wiring bug, and is worth revisiting once real object meshes replace the flat-colored placeholder — FLIP was trained on real-object imagery.
- **ArUco detection is placement-sensitive.** Across a small evaluation sweep, the marker went undetected in some trials purely from placement variation within the current sampling range.
- **Fixed target rotation through Phase 6.** The ArUco decal only renders correctly on an un-rotated top face — a MuJoCo texture-mapping limitation, documented at its fix site in `environment.py`.

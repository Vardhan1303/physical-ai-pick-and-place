...
- `TableArena` + Panda robot, one `BoxObject` target (placeholder — later phases swap in bottle/can/cup meshes; nothing here hard-codes "box" as a perception assumption, it's only a Phase 1 scene-construction convenience), 0 distractors by default, a fixed destination-bin `BoxObject` with no free joint.
...
- Deterministic seeding via robosuite's own `seed=` kwarg → `self.rng`, passed into `BoxObject(..., rng=self.rng)` and `UniformRandomSampler(..., rng=self.rng)`.
